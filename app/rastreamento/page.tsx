...
export default function RastreamentoPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#0b0b14' }}>
      ...
      <main className="mx-auto max-w-[1200px] px-6 py-8">
        <div className="mb-6">
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Cadastre aqui os dados do Meta Pixel + CAPI e do webhook de compra. Nesta etapa é só cadastro —
            o deploy real do Worker na Cloudflare vem numa próxima etapa.
          </p>
        </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400" style={{ background: 'rgba(255,255,255,0.06)' }}>
                    ...
                    <p className="mt-1 text-[11px] text-slate-600">
                      {inst.pixels.length} pixel{inst.pixels.length !== 1 ? 's' : ''} · {inst.domains.length} domínio{inst.domains.length !== 1 ? 's' : ''} · {inst.triggers.length} gatilho{inst.triggers.length !== 1 ? 's' : ''}
                    </p>
                  </div>
      ...
      <InstallationModal open={showModal} installation={editing} onClose={() => setShowModal(false)} onSaved={handleSaved} />
